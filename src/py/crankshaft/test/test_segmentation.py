...
class SegmentationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model_data = json.loads(open(fixture_file("model_data.json")).read())
        self.data = json.loads(open(fixture_file("data.json")).read())
        self.predict_data = json.loads(open(fixture_file("predict_data.json")).read())
        self.result_seg = json.loads(
            open(fixture_file("segmentation_result.json")).read()
        )
        self.true_result = json.loads(open(fixture_file("true_result.json")).read())
    ...
    def test_create_and_predict_segment(self):
        ...
        feat = np.column_stack(
            [np.array(self.model_data[0][col]) for col in feature_columns]
        ).astype(float)
        ...
        batches = np.row_stack([np.array(row["features"]) for row in cursor]).astype(
            float
        )
        ...
        data_predict = [
            OrderedDict([("features", d["features"]), ("batch", batch)])
        ...
        data = [OrderedDict([("ids", d["ids"])]) for d in self.data]

        seg = Segmentation(RawDataProvider(data, data_model, data_predict))
        ...
        accuracy = np.sqrt(np.mean((np.array(prediction) - np.array(pre_res)) ** 2))
