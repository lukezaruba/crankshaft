...
class Kmeans(object):
    ...
    def spatial(self, query, no_clusters, no_init=20):
        ...
        """
        params = {"subquery": query, "geom_col": "the_geom", "id_col": "cartodb_id"}
    ...
    def nonspatial(
        self, subquery, colnames, no_clusters=5, standardize=True, id_col="cartodb_id"
    ):
        """
        Arguments:
        ...
        kmeans = KMeans(n_clusters=no_clusters, random_state=0).fit(cluster_columns)

        centers = [
            json.dumps(dict(list(zip(colnames, c))))
            for c in kmeans.cluster_centers_[kmeans.labels_]
        ]

        silhouettes = metrics.silhouette_samples(
            cluster_columns, kmeans.labels_, metric="sqeuclidean"
        )
...
def _extract_columns(data):
    ...
    n_cols = len(data[0]) - 1
    return np.array(
        [data[0]["arr_col{0}".format(i + 1)] for i in range(n_cols)], dtype=float
    ).T
