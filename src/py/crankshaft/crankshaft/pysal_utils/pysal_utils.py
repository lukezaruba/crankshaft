...
def get_weight(query_res, w_type="knn", num_ngbrs=5):
    ...
    built_weight = libpysal.weights.W(neighbors)
...
def query_attr_select(params, table_ref=True):
    ...
    if ("time_cols" in params) or ("ind_vars" in params):
        ...
        attrs = params["time_cols"] if "time_cols" in params else params["ind_vars"]
...
def query_attr_where(params, table_ref=True):
    ...
    if ("time_cols" in params) or ("ind_vars" in params):
        ...
        if "denominator" in attrs:
            attr_string.append('idx_replace."%s" <> 0' % params["denominator"])
...
def gwr_query(params):
    ...
    replacements = {
        "ind_vars_select": query_attr_select(params, table_ref=None),
        "ind_vars_where": query_attr_where(params, table_ref=None),
    }
