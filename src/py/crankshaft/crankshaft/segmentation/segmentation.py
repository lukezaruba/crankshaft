...
class Segmentation(object):
    ...
    def create_and_predict_segment_agg(
        self, target, features, target_features, target_ids, model_parameters
    ):
        """
    ...
    def create_and_predict_segment(
        ...
        """
        params = {"subquery": target_query, "id_col": id_col}

        (target, features, target_mean, feature_means) = self.clean_data(
            query, variable, feature_columns
        )

        model, accuracy = train_model(target, features, model_params, 0.2)
        result = self.predict_segment(
            model, feature_columns, target_query, feature_means
        )
    ...
    def predict_segment(self, model, feature_columns, target_query, feature_means):
        ...
        batch_size = 1000
        params = {"subquery": target_query, "feature_columns": feature_columns}
        ...
        while True:
            ...
            if not rows:
                break
            batch = np.row_stack([np.array(row["features"]) for row in rows]).astype(
                float
            )
    ...
    def clean_data(self, query, variable, feature_columns):
        ...
        Add docstring
        """
        params = {"subquery": query, "target": variable, "features": feature_columns}
        ...
        features = np.column_stack(
            [np.array(data[0][col]) for col in feature_columns]
        ).astype(float)
...
def train_model(target, features, model_params, test_split):
    ...
    features_train, features_test, target_train, target_test = train_test_split(
        features, target, test_size=test_split
    )
